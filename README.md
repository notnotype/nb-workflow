...
当前 npm Registry 最新版本为 `0.2.0`，包含 Deferred Activity 和 package gate 加固。Memory 组合用于测试、demo 和 Backend conformance；它不支持
进程重启或多 Worker，也不等同真实 durable host、跨进程恢复或 Cosmos 生产集成。
...
- `bun run verify:package` 额外运行仓库声明 consumer、真实 tarball 的
  NodeNext declaration consumer（只做导出类型检查）、纯 Node import/execute
  smoke，以及隔离目录 smoke（在系统临时目录安装 tarball，hostile `NODE_PATH`，
  执行 Deferred Activity pending/completion/conflict/late-completion 行为）。
- `npm publish` 先运行 `prepublishOnly`；它执行 `bun run verify:package`（包含构建），
  再检查发布元数据、dist 和 tarball 内容。gate 拒绝 README/package.json/LICENSE
  存在未提交差异，或 dist/tarball 内容不完整。
...
`DeferredActivityExecutor` 是可选宿主端口：`startAction()` 返回已完成结果，或返回不透明 `receipt` 使 Run 进入 waiting。宿主必须按 `context.idempotencyKey` 幂等创建外部工作，或能用该 key 重新发现已有工作，因为 Kernel 可能在 receipt 落库前崩溃。宿主随后通过 `WorkflowRunner.completeActivity()` 提交包含 activity key、receipt、reference 和 fingerprint 的 completion。成功结果写入原 Activity journal；相同 completion 幂等，不同 completion 冲突，取消或终态后的迟到 completion 被拒绝。Memory Backend 仍不支持真实进程恢复、多 Worker 或 durable external receipt。Task 03 当前 worktree 的本地 tarball smoke 不是 npm Registry 包 smoke，也不改变 Registry 的 `0.1.2`。
